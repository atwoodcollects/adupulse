...
export default function PlymouthPage() {
  ...
  const stats = {
    ...
    avgPpsf: permitsWithCost.length > 0 ? Math.round(permitsWithCost.reduce((a, b) => a + (b.sqft > 0 ? b.cost / b.sqft : 0), 0) / permitsWithCost.filter(p => p.sqft > 0).length) : 0
  }
  ...
  const subtypeData = [
    { name: 'Basement', value: permits.filter(p => p.subtype === 'Basement').length, color: '#10b981' },
    { name: 'Garage', value: permits.filter(p => p.subtype === 'Garage').length, color: '#3b82f6' },
    { name: 'Barn', value: permits.filter(p => p.subtype === 'Barn').length, color: '#f59e0b' },
    { name: 'Dwelling', value: permits.filter(p => p.subtype === 'Dwelling').length, color: '#ec4899' },
    { name: 'Other', value: permits.filter(p => p.subtype === 'Accessory Bldg').length, color: '#8b5cf6' },
  ].filter(d => d.value > 0)

  const monthlyData = [
    { month: 'Jan', permits: permits.filter(p => p.issued.startsWith('2025-01')).length },
    { month: 'Feb', permits: permits.filter(p => p.issued.startsWith('2025-02')).length },
    { month: 'Mar', permits: permits.filter(p => p.issued.startsWith('2025-03')).length },
    { month: 'Apr', permits: permits.filter(p => p.issued.startsWith('2025-04')).length },
    { month: 'May', permits: permits.filter(p => p.issued.startsWith('2025-05')).length },
    { month: 'Jun', permits: permits.filter(p => p.issued.startsWith('2025-06')).length },
    { month: 'Jul', permits: permits.filter(p => p.issued.startsWith('2025-07')).length },
    { month: 'Aug', permits: permits.filter(p => p.issued.startsWith('2025-08')).length },
    { month: 'Sep', permits: permits.filter(p => p.issued.startsWith('2025-09')).length },
    { month: 'Oct', permits: permits.filter(p => p.issued.startsWith('2025-10')).length },
    { month: 'Nov', permits: permits.filter(p => p.issued.startsWith('2025-11')).length },
    { month: 'Dec', permits: permits.filter(p => p.issued.startsWith('2025-12')).length },
  ]
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      <div className="max-w-6xl mx-auto px-3 sm:px-4 md:px-6 py-3 sm:py-4 md:py-6">
        ...
        <div className="bg-gray-800/50 border border-border rounded-lg p-3 sm:p-4 mb-4 sm:mb-6">
          <h3 className="text-white text-sm sm:text-base font-medium mb-3">Monthly Permits (2025)</h3>
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={monthlyData}>
              ...
              <Bar dataKey="permits" fill="#f59e0b" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-4 sm:mb-6">
          ...
          <div className="bg-gray-800/50 border border-border rounded-lg p-3 sm:p-4">
            <h3 className="text-white text-sm sm:text-base font-medium mb-3">Conversion Types</h3>
            <ResponsiveContainer width="100%" height={140}>
              <PieChart>
                <Pie data={subtypeData} cx="50%" cy="50%" innerRadius={30} outerRadius={50} dataKey="value" label={({ name, value }) => name + ': ' + value} labelLine={false}>
                  {subtypeData.map((entry, index) => (<Cell key={'cell-' + index} fill={entry.color} />))}
                </Pie>
                <Tooltip contentStyle={{ backgroundColor: '#1f2937', border: '1px solid #374151', borderRadius: '8px', fontSize: '12px' }} />
              </PieChart>
            </ResponsiveContainer>
          ...
          <div className="bg-gray-800/50 border border-border rounded-lg p-3 sm:p-4">
            ...
            <div className="space-y-2 text-xs sm:text-sm">
              <div className="flex justify-between"><span className="text-text-secondary">Size Range</span><span className="text-white">184-1,000 sf</span></div>
              <div className="flex justify-between"><span className="text-text-secondary">Cost Range</span><span className="text-white">$125K-$463K</span></div>
              <div className="flex justify-between"><span className="text-text-secondary">Denials</span><span className="text-red-400">2</span></div>
            </div>
          </div>
        </div>
        ...
        <p className="text-text-muted text-[10px] sm:text-xs mt-3 sm:mt-4 text-center">Data source: Town of Plymouth Permit Reports (2025)</p>
      </div>
    </div>
  )
}
